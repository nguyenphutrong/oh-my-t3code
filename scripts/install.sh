...
if "$interactive"; then printf '\r\033[2K' >&2; fi
printf '  %sInstalled Oh My T3Code %s%s\n\n' "$green" "$version" "$reset" >&2
case ":${PATH}:" in
  *":${bin_dir}:"*) printf '  Run %soh-my-t3code%s to get started.\n\n' "$bold" "$reset" ;;
  *) printf '  Add %s to your PATH, then run %soh-my-t3code%s.\n\n' "$bin_dir" "$bold" "$reset" ;;
esac
