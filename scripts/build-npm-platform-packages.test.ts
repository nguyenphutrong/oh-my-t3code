...
/** A tar.gz laid out like build-cli-archive.ts writes, with a stub executable that echoes args. */
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  ...
  for (const key of KEYS) {
    const stem = `oh-my-t3code-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "oh-my-t3code"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "oh-my-t3code"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@oh-my-t3code/cli-darwin-arm64", "@oh-my-t3code/cli-linux-x64", "oh-my-t3code"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@oh-my-t3code/cli-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@oh-my-t3code/cli-linux-x64");
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@oh-my-t3code/cli-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/oh-my-t3code$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(
          process.execPath,
          ["bin/oh-my-t3code.js", "serve", "--port", "1234"],
          { cwd: launcherDir, env },
        );
        assert.equal(
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "oh-my-t3code.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/oh-my-t3code.js", "--version"], {
        cwd: launcherDir,
