...
describe.skipIf(HostProcessPlatform.defaultValue() !== "linux")("installer terminal", () => {
  it.each([false, true])(
    ...
    async (fail) => {
      ...
      const version = "1.2.3";
      const stem = `oh-my-t3code-${version}-linux-${HostProcessArchitecture.defaultValue()}`;
      ...
      await NodeFSP.writeFile(
        NodePath.join(root, stem, "oh-my-t3code"),
        "#!/bin/sh\necho 'oh-my-t3code v1.2.3'\n",
        { mode: 0o755 },
      );
      await NodeFSP.writeFile(
      ...
      try {
        ...
        if (fail) {
          ...
          expect(output).not.toContain("Installed Oh My T3Code");
          ...
          expect(output).toContain("Installed Oh My T3Code 1.2.3");
          expect(
            await NodeFSP.readFile(NodePath.join(versions, version, ".install-complete"), "utf8"),
          ).toBe("1.2.3\n");
          expect(
            NodeChildProcess.execFileSync(NodePath.join(root, "bin/oh-my-t3code"), ["--version"], {
              encoding: "utf8",
            }).trim(),
          ).toBe("oh-my-t3code v1.2.3");
