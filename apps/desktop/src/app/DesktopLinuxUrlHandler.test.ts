...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "Oh My T3Code (Alpha)",
    linuxDesktopEntryName: "app.bytrong.ohmyt3code.desktop",
    linuxWmClass: "oh-my-t3code",
    linuxApplicationsDir: "/home/alice/.local/share/applications",
    appImagePath: Option.some("/home/alice/Applications/Oh-My-T3Code.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: "Oh My T3Code (Nightly)",
      execTarget: '/home/al ice/Apps/Oh My T3Code "100%" $HOME\\x.AppImage',
      scheme: "oh-my-t3code",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=Oh My T3Code (Nightly)");
    ...
    assert.include(
      entry,
      'Exec="/home/al ice/Apps/Oh My T3Code \\\\"100%%\\\\" \\\\$HOME\\\\\\\\x.AppImage" %U',
    );
    assert.include(entry, "NoDisplay=true");
    assert.notInclude(entry, "StartupWMClass=");
    assert.include(entry, "MimeType=x-scheme-handler/oh-my-t3code;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "oh-my-t3code",
      desktopEntryPath: "/home/alice/.local/share/applications/app.bytrong.ohmyt3code.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the oh-my-t3code:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/app.bytrong.ohmyt3code.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the oh-my-t3code:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        ...
        'Exec="/home/alice/Applications/Oh-My-T3Code.AppImage" %U',
      );
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/oh-my-t3code;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "app.bytrong.ohmyt3code.desktop", "x-scheme-handler/oh-my-t3code"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      yield* runRegister(recorded, {
        existingEntry: DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
          displayName: "Oh My T3Code (Alpha)",
          execTarget: "/home/alice/Applications/Oh-My-T3Code.AppImage",
          scheme: "oh-my-t3code",
        }),
      });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(unpackaged, {
        environment: {
          isPackaged: false,
          linuxDesktopEntryName: "app.bytrong.ohmyt3code.Development.desktop",
        },
      });
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/app.bytrong.ohmyt3code.Development.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/app.bytrong.ohmyt3code.desktop",
        }),
      });
