...
describe("ElectronProtocol", () => {
  ...
  it.effect("serves the bundled client from disk without a backend", () =>
    Effect.gen(function* () {
      ...
      const request = (pathname: string, init?: RequestInit) =>
        Effect.promise(() => handler!(new Request(`oh-my-t3code://app${pathname}`, init)));
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("oh-my-t3code-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "oh-my-t3code-dev://app",
                  referer: "oh-my-t3code-dev://app/",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' oh-my-t3code-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' oh-my-t3code-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["oh-my-t3code-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("oh-my-t3code://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("oh-my-t3code-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "oh-my-t3code-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to register Electron protocol scheme "oh-my-t3code-dev".',
      );
    }).pipe(Effect.provide(protocolLayer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "oh-my-t3code");
        assert.strictEqual(error.cause, cause);
        assert.equal(
          error.message,
          'Failed to unregister Electron protocol scheme "oh-my-t3code".',
        );
      }
    }).pipe(Effect.provide(protocolLayer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["font-src"], ["'self'", "oh-my-t3code:", "data:"]);
  });
});
