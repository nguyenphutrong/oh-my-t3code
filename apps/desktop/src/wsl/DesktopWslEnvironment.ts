...
export class DesktopWslEnvironment extends Context.Service<
  DesktopWslEnvironment,
  {
    ...
    // Proves a staged self-contained runtime can run (`<root>/oh-my-t3code --version`)
...
// Holds the sha256 of the runtime's `oh-my-t3code` executable, written when the install
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    ...
    // The runtime is a self-contained `oh-my-t3code` executable with Node inside, so the
    // readiness proof is the same one the SSH runner and the CLI installers
    // use: the file is executable and `oh-my-t3code --version` exits 0. That covers the
    ...
    "}",
    // Hashing the entry is what tells a working cache from one whose executable was
    ...
    `  sha256sum "$1/oh-my-t3code" 2>/dev/null | cut -d ' ' -f 1`,
    "}",
    "runtime_is_ready() {",
    '  [ -f "$ready_marker" ] &&',
    ...
    // The release archive has one top-level `oh-my-t3code-<version>-linux-<arch>/`
    // directory; strip it so the executable lands at `$runtime_root/oh-my-t3code`.
    ...
    "  printf 'WSL runtime archive does not contain a working oh-my-t3code executable\\n' >&2",
    "  exit 1",
    "fi",
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.oh-my-t3code/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? "WSL support is missing from this Oh My T3Code build: the packaged Linux node-pty binary was not included. Install a build that includes WSL support."
    : null;
...
export const buildWslRuntimeProbeScript = (linuxAppRoot: string) =>
  [
    ...
    `${shellQuote(`${linuxAppRoot}/oh-my-t3code`)} --version >/dev/null 2>&1`,
  ].join("\n");
...
const probeWslRuntimeImpl = (
  ...
  Effect.gen(function* () {
    ...
    if (probe.exitCode !== 0) {
      ...
      return {
        ok: false,
        reason: `${linuxAppRoot}/oh-my-t3code --version failed (exit ${probe.exitCode})${trimmedTail ? `: ${trimmedTail}` : ""}`,
      } as const;
    }
