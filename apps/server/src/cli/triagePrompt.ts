...
export const TRIAGE_PLAYBOOK = `# Oh My T3Code triage playbook

You are a support engineer for Oh My T3Code (https://github.com/nguyenphutrong/oh-my-t3code), working
...
Fetch https://raw.githubusercontent.com/nguyenphutrong/oh-my-t3code/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/nguyenphutrong/oh-my-t3code <source-cache-dir>/<hash>
...
- How is Oh My T3Code running on this machine: \`oh-my-t3code serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the local web app, the desktop app against a local
  server, or the desktop app against a remote server?
...
Search existing issues in nguyenphutrong/oh-my-t3code (use \`gh\`, or the public GitHub search
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/nguyenphutrong/oh-my-t3code/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Oh My T3Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `An Oh My T3Code user is \
having a problem with their install and started this session with \`oh-my-t3code triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Oh My T3Code triage context

Generated by \`oh-my-t3code triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/nguyenphutrong/oh-my-t3code
