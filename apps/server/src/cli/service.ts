...
export function formatServiceStatus(
  ...
  if (!status.supported) {
    return "Oh My T3Code service\n  Status: unavailable on this machine\n  Supported on: Linux with systemd, macOS with launchd";
  }
  if (!status.installed) {
    return "Oh My T3Code service\n  Status: not installed\n  Next: Run `oh-my-t3code service install`.";
  }
  ...
  if (
    ...
    return [
      ...
      `  Status: installed · ${installedVersion} (newer than this ${cliVersion} CLI)`,
      `  Unit: ${status.unitPath}`,
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Run \`oh-my-t3code update ${installedVersion}\` to match it, or pass \`--allow-downgrade\` to \`oh-my-t3code service install\` explicitly.`,
    ].join("\n");
  }
  return [
    "Oh My T3Code service",
    `  Status: ${status.current ? `installed · ${installedVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : ["  Next: Run `oh-my-t3code service install` to repair it."]),
  ].join("\n");
}
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Oh My T3Code service is already installed with ${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Oh My T3Code service with ${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription("Deprecated. Run `oh-my-t3code update` to move to a newer release."),
  Command.unlisted,
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log(
          "`oh-my-t3code service update` is deprecated: run `oh-my-t3code update` to move to a newer release, or `oh-my-t3code service install` to repair the service. Repairing now.",
        );
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(`Oh My T3Code service is already using ${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Oh My T3Code service with ${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceRestartCommand = Command.make("restart", projectLocationFlags).pipe(
  Command.withDescription(
    "Restart the background service. Picks up a version installed by `oh-my-t3code update` that was not restarted at the time.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          restarted
            ? `Restarted the Oh My T3Code service${status.installedVersion === undefined ? "" : ` on ${status.installedVersion}`}.`
            : "Oh My T3Code service is not installed.",
        );
      }),
    ),
  ),
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the Oh My T3Code service." : "Oh My T3Code service is not installed.",
        );
      }),
    ),
  ),
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (
    ...
    yield* Console.log(
      `A newer Oh My T3Code ${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed Oh My T3Code service needs an update or repair. Update it now?"
        : platform === "darwin"
