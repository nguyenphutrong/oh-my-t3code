...
it("launch prompt stays a single argv-safe line naming the prompt file", () => {
  ...
  const launch = buildTriageLaunchPrompt(
    String.raw`C:\Users\a b\.oh-my-t3code\userdata\triage\x\prompt.md`,
  );
  assert.notInclude(launch, "\n");
  assert.include(launch, String.raw`C:\Users\a b\.oh-my-t3code\userdata\triage\x\prompt.md`);
...
it("context file carries every path the playbook depends on", () => {
  const context = buildTriageContext({
    ...
    os: "linux x64 (7.0.0)",
    nodeVersion: "v24.0.0",
    launchedAs: "oh-my-t3code triage",
    server: "running (pid 42, http://127.0.0.1:4501)",
    paths: {
      stateDir: "/home/u/.oh-my-t3code/userdata",
      ...
      providerStatusCacheDir: "/home/u/.oh-my-t3code/caches",
      secretsDir: "/home/u/.oh-my-t3code/userdata/secrets",
      sourceCacheDir: "/home/u/.oh-my-t3code/source",
    },
  });
  ...
  assert.include(context, "oh-my-t3code triage");
