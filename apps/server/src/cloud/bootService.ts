...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    "[Unit]",
    "Description=Oh My T3Code server",
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `oh-my-t3code service install` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status oh-my-t3code.service`, then run `oh-my-t3code service install`.";
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `oh-my-t3code service restart` to switch.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace Oh My T3Code ${this.installedVersion} with older ${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
