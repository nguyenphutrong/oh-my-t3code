...
describe("AcpRegistrySupport", () => {
  ...
  it.effect("searches compatible agents with deterministic ranking and bounded metadata", () => {
    ...
    const amp = {
      ...makeAgent({ npx: { package: "@sourcegraph/amp@1.2.3" } }),
      id: "amp",
      name: "Amp",
      description: "Amp agent for Codex workflows",
    } satisfies AcpRegistryAgent;
    ...
    return Effect.gen(function* () {
      ...
      expect(result.agents.map((agent) => agent.id)).toEqual(["amp", "codex-acp", "other-agent"]);
          ...
          Effect.succeed(
            HttpClientResponse.fromWeb(
              ...
              new Response(
                JSON.stringify({
                  version: "1.0.0",
                  agents: [descriptionMatch, incompatible, exact, amp],
                }),
              ),
            ),
          ),
  ...
  it.effect("rejects malformed registry manifests and discards unsafe URLs", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cacheDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-acp-registry-malformed-",
      });
      const resolver = yield* makeAcpRegistryCatalog({ cacheDir, registryUrl });
      ...
      Effect.provide(
        resolverLayer((request) =>
          Effect.succeed(
            HttpClientResponse.fromWeb(
              request,
              new Response(JSON.stringify({ version: "1.0.0", agents: "not-an-array" })),
            ),
          ),
        ),
      ),
  ...
  it.effect("does not expose entries with credentialed or non-HTTPS archive URLs", () => {
    const unsafeAgents = [
      makeAgent({
        binary: { "linux-x86_64": { archive: "http://registry.test/agent", cmd: "agent" } },
      }),
      {
        ...makeAgent({
          binary: {
            "linux-x86_64": { archive: "https://u:p@registry.test/agent", cmd: "agent" },
          },
        }),
        id: "credentialed",
      },
    ...
    return Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cacheDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-acp-registry-url-",
      });
      const resolver = yield* makeAcpRegistryCatalog({ cacheDir, registryUrl });
      expect((yield* resolver.search({ query: "" })).agents).toEqual([]);
    }).pipe(
      Effect.scoped,
      Effect.provide(
        resolverLayer((request) =>
          Effect.succeed(
            HttpClientResponse.fromWeb(
              request,
              new Response(JSON.stringify({ version: "1.0.0", agents: unsafeAgents })),
            ),
          ),
        ),
      ),
