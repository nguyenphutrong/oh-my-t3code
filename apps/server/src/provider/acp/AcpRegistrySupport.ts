...
> {
  ...
  const search: AcpRegistryCatalog["Service"]["search"] = (input) =>
    Effect.gen(function* () {
      ...
      ranked.sort(
        (left, right) =>
          (left.agent.id === "amp" || left.agent.id === "amp-acp" ? -1 : 0) -
            (right.agent.id === "amp" || right.agent.id === "amp-acp" ? -1 : 0) ||
  ...
  const resolve: AcpRegistryCatalog["Service"]["resolve"] = (settings, cwd, environment) =>
    Effect.gen(function* () {
      ...
      const commandOverride = settings.commandPath.trim();
      const launchArgsOverride = settings.launchArgs.trim();
      if (commandOverride.length > 0) {
        ...
        args =
          launchArgsOverride.length > 0 ? tokenizeCliArgs(launchArgsOverride) : distribution.args;
