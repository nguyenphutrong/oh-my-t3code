...
const buildExeCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* Effect.log(
        "[cli] Built dist-exe/oh-my-t3code (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)",
      );
    }),
).pipe(
 ...
 * every `@oh-my-t3code/cli-<platform>.tgz` first, `oh-my-t3code.tgz` (the
 * launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@oh-my-t3code");
      const launcherTarball = path.join(packagesDir, "oh-my-t3code.tgz");
  ...
  Command.withDescription(
    "Publish the @oh-my-t3code/cli-<platform> tarballs and then the oh-my-t3code launcher to npm.",
  ),
);
