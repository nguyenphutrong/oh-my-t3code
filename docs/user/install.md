...
To import new T3 Code state again, stop both apps and run `oh-my-t3code import-t3`, then restart
Oh My T3Code. The command adds source-only projects, threads, providers, and usage sources without
overwriting existing Oh My T3Code entries. Re-running it is safe, but an existing thread never
receives newer messages from the matching T3 Code thread.
...
| Amp          | Install [Amp](https://ampcode.com/manual#installation), then authenticate with Amp.          |
...
[Claude](./providers-claude.md), [Amp](./providers-amp.md), [OpenCode](./providers-opencode.md), and
